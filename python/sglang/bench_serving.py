...
class RequestFuncOutput:
    ...
    # FLAT_MEMORY: Missing detail fields are not cold hits; mixed is an SSD subset.
                    ...
                    async for chunk_bytes in response.content:
                        ...
                        if chunk == "[DONE]":
                            ...
                            if details:
                                ...
                                # FLAT_MEMORY: Streaming details are cumulative, not deltas.
                                output.server_cached_tokens_device = details.get("device")
                                output.cache_source_mode = details.get("cache_source_mode")
                                output.tiered_cache = details.get("tiered_cache")
                                for name in (
                                    "flat_dram", "flat_ssd", "flat_mixed",
                                    "flat_prefetch_dram_ms", "flat_prefetch_dram_ops",
                                    "flat_prefetch_ssd_ms", "flat_prefetch_ssd_ops",
                                ):
                                    setattr(output, name, details.get(name))
...
# FLAT_MEMORY: Native measurements use completed I/O, not eviction-based estimates.
def fetch_hicache_io_snapshot(base_url: str, timeout: float = 120.0) -> dict:
    ...
    while True:
        ...
        response = requests.get(
            base_url + "/server_info", headers=get_auth_headers(), timeout=min(5.0, remaining)
        )
        ...
        if (
            ...
            or info.get("dp_size") != 1 or info.get("pp_size") != 1
        ):
        ...
        if (type(tp_size) is not int or tp_size < 1 or not isinstance(states, list)
                or len(states) != 1 or not isinstance(states[0], dict)):
        ...
        for rank in ranks:
            ...
            if (rank["tp_rank"] not in range(tp_size) or rank["tp_rank"] in seen
                    or rank["tp_size"] != tp_size or rank.get("enabled") is not True
                    or rank.get("io_backend") != "direct" or type(rank.get("idle")) is not bool):
...
def calculate_hicache_io_metrics(before: dict, after: dict) -> dict:
    ...
    result["hicache_io_metadata"] = {
        ...
        "totals": totals, "rank_deltas": deltas, "before": before, "after": after,
    }
    return result
...
def print_mooncake_io_metrics(metrics: dict) -> None:
    ...
    for medium, prefix in (("DRAM", "mooncake_dram"), ("Owner SSD", "ssd")):
        for direction in ("write", "read"):
            ...
            if prefix == "ssd":
                rows.append((f"Owner SSD {direction} bandwidth peak (GB/s, 100 ms):", f"ssd_{direction}_peak_bw_gbps", 1))
...
# FLAT_MEMORY: Reporting helpers consume the server contract without inferring I/O.
...
# FLAT_MEMORY: Media totals count logical request tokens, not TP shards or read bytes.
def summarize_tiered_cache(outputs):
    ...
    if not successful:
        result.update(total_cached_tokens=0, total_prompt_tokens=0,
    ...
    try:
        for index, output in enumerate(successful):
            prompt = counter(output.server_prompt_tokens, f"prompt tokens for successful request {index}")
            cached = counter(output.server_cached_tokens, f"cached tokens for successful request {index}")
            if cached > prompt:
                raise ValueError(f"Cached tokens exceed prompt tokens for successful request {index}")
        prompt = result["total_prompt_tokens"]
        if prompt:
            result["cache_hit_rate"] = result["total_cached_tokens"] / prompt
        totals = dict.fromkeys(fields, 0)
        for index, output in enumerate(successful):
            if output.cache_source_mode != "mooncake_tiered_gds" or not isinstance(output.tiered_cache, dict):
                raise ValueError(f"Missing Mooncake GDS source metadata for successful request {index}; restart with matching SGLang code")
            counts = {name: counter(output.tiered_cache.get(name), f"{name} count for successful request {index}") for name in fields}
            if (counts["host"] != counts["l2_host"] + counts["mooncake_dram"]
                    or counts["mixed"] > counts["ssd"]
                    or counts["device"] + counts["host"] + counts["ssd"] != output.server_cached_tokens
                    or counts["device"] != output.cached_tokens_device
                    or counts["l2_host"] != output.cached_tokens_host
                    or counts["mooncake_dram"] + counts["ssd"] != output.cached_tokens_storage):
                raise ValueError(f"Cache source totals are inconsistent for successful request {index}")
            for name in fields:
                totals[name] += counts[name]
        result.update({f"tiered_cached_tokens_{name}": value for name, value in totals.items()})
        if not prompt:
...
def print_tiered_cache(summary):
    ...
    for label, key in (
        ...
        ("Host detail: Mooncake DRAM-only tokens", "tiered_cached_tokens_mooncake_dram"),
        ("SSD-dependent tokens", "tiered_cached_tokens_ssd"),
        ("Mixed tokens (subset of SSD)", "tiered_cached_tokens_mixed"),
    ):
        value = summary[key]
        rendered = "N/A" if value is None else (f"{value:.2f}%" if key.endswith("_pct") else str(value))
...
def summarize_flat_cache(outputs, total_prompt_tokens):
    ...
    if total is not None and (
        total != sum(output.cached_tokens_storage for output in successful)
        or mixed is None or mixed > ssd
    ):
        dram = ssd = mixed = total = None
    device = _flat_optional_sum(successful, "cached_tokens_device")

    def rate(tokens):
        return (
            100 * tokens / total_prompt_tokens
            if tokens is not None and total_prompt_tokens > 0 else None
        )
...
_FLAT_WINDOW_COUNTERS = (
    "read_completed_ops", "read_completed_bytes", "write_completed_ops",
    "write_completed_bytes", "durable_write_batches", "durable_write_bytes",
    "io_errors",
)
...
def _validate_flat_window(snapshot, window_id, active):
    ...
    for rank in ranks.values():
        ...
        start = _flat_counter(window["start_ns"])
        end = _flat_counter(window["end_ns"])
        if not start or (not active and end <= start):
            raise ValueError("Invalid Flat window boundaries")
        boundaries.add((start, end))
        for name in _FLAT_WINDOW_COUNTERS:
        ...
        if not isinstance(window["buckets"], list):
            raise ValueError("Missing Flat I/O buckets")
...
def summarize_flat_io(before, after, window_id):
    ...
    totals = {name: 0 for name in _FLAT_WINDOW_COUNTERS}
    deltas = {name: 0 for name in (
        "dram_read_total_bytes", "dram_write_total_bytes", "ssd_write_count"
    )}
    buckets = {}
    for tp_rank, rank in last.items():
        ...
        error_delta = _flat_counter(rank["flat_io_errors"] - prior["flat_io_errors"])
        if error_delta:
        ...
        for bucket in window["buckets"]:
            ...
            seen.add(index)
            read = _flat_counter(bucket["read_bytes"])
            write = _flat_counter(bucket["write_bytes"])
            read_bytes += read
            write_bytes += write
            merged = buckets.setdefault(index, [0, 0])
            merged[0] += read
            merged[1] += write
        if (read_bytes != window["read_completed_bytes"]
    ...
    return {
        ...
        # Merge by index first; even the last partial bucket has a 0.1s divisor.
        "flat_ssd_read_peak_bw_gbps": max((b[0] for b in buckets.values()), default=0) / 0.1 / 1e9,
        "flat_ssd_write_peak_bw_gbps": max((b[1] for b in buckets.values()), default=0) / 0.1 / 1e9,
...
def validate_gds_io_window(snapshot, window_id, active):
    ...
    for rank in ranks:
        ...
        if (rank["tp_rank"] not in range(len(ranks)) or rank["tp_rank"] in mapped
                or rank["pid"] in pids or rank["tp_size"] != len(ranks)
        ...
        if (not isinstance(window, dict) or type(window.get("id")) is not int
                or window["id"] != int(window_id) or window.get("active") is not active
                or window.get("capture_buckets") is not True or window.get("aborted") is not False
                or window.get("overflowed") is not False or type(window.get("bucket_ns")) is not int
                or window["bucket_ns"] != 100_000_000):
            raise ValueError("Mismatched, aborted, overflowing or unsupported GDS window")
        start, end = window.get("start_ns"), window.get("end_ns")
        if (type(start) is not int or start <= 0 or start > rank["sampled_at_ns"]
                or type(end) is not int or (active and end != 0)
        ...
        for bucket in buckets:
            ...
            if (type(index) is not int or not 0 <= index < 72000 or index in seen
...
def unavailable_gds_io_metrics(error):
    result = dict.fromkeys((
        "gds_io_window_seconds", "gds_ssd_read_bw_gbps", "gds_ssd_read_peak_bw_gbps",
        "gds_ssd_read_ops", "gds_ssd_read_total_bytes", "gds_ssd_read_errors",
    ))
...
def calculate_gds_io_metrics(before, after, window_id):
    ...
    for rank_id, rank in last.items():
        ...
        if (any(initial[key] != rank[key] for key in ("pid", "tp_size", "gpu_id", "generation"))
        ...
        for key in totals:
            if rank["totals"]["ssd_read"][key] - initial["totals"]["ssd_read"][key] < counters[key]:
                raise ValueError("GDS lifetime counters reset or disagree with the window")
    ...
    return {
        "gds_io_status": "ok", "gds_io_error": None,
        ...
        "gds_ssd_read_ops": totals["ops"], "gds_ssd_read_total_bytes": totals["bytes"],
        "gds_ssd_read_errors": totals["errors"],
        "gds_io_metadata": {
            "window_id": str(window_id), "start_ns": end[0], "end_ns": end[1], "bucket_ns": 100_000_000,
            ...
            "buckets": [{"index": index, "ssd_read_bytes": value} for index, value in sorted(buckets.items())],
            "before": before, "after": after,
        },
    }


def fetch_gds_io_window(base_url, action, window_id, timeout=150):
    response = requests.post(
        base_url + "/mooncake/gds_io_window", headers=get_auth_headers(),
        json={"action": action, "window_id": str(window_id)}, timeout=timeout,
    )
    response.raise_for_status()
...
# FLAT_MEMORY: Only snapshots/window control live here; request timing stays in benchmark().
class NativeIOWindow:
    def __init__(self, base_url, backend):
        self.base_url = base_url
        self.collect_mooncake = bool(getattr(args, "collect_mooncake_io_metrics", False))
        self.collect_gds = bool(getattr(args, "collect_mooncake_gds_io", False))
        self.collect_host = self.collect_mooncake or self.collect_gds or bool(getattr(args, "collect_hicache_io_metrics", False))
        self.before = self.after = self.ssd_before = self.ssd_after = None
        self.gds_before = self.gds_after = None
        self.gds_window_id = str(uuid.uuid4().int % (2**63 - 1) + 1) if self.collect_gds else None
        ...
        if not self.collect_host:
            return
        if backend != "sglang":
            raise ValueError("Native HiCache/Mooncake I/O metrics require --backend sglang")
        if getattr(args, "collect_flat_memory_io", False):
            raise ValueError("Native Mooncake/HiCache I/O and Flat I/O windows cannot be combined")
        ...
        if not (math.isfinite(self.timeout) and self.timeout > 0
                and math.isfinite(self.flush_timeout) and self.flush_timeout > self.timeout):
        ...
        if self.collect_mooncake:
            ...
            self.client_url = f"http://{args.mooncake_client_host}:{args.mooncake_client_metrics_port}"
            fetch_mooncake_io_snapshot(self.client_url)

    def _abort_gds(self):
        if self.gds_begin_attempted:
            try:
                fetch_gds_io_window(self.base_url, "abort", self.gds_window_id, self.flush_timeout)
            except (requests.RequestException, ValueError, TimeoutError) as error:
                warnings.warn(f"Could not abort GDS I/O window {self.gds_window_id}: {error}")
            finally:
                self.gds_begin_attempted = False
    ...
    async def __aenter__(self):
        try:
            if self.collect_host:
                self.before = fetch_hicache_io_snapshot(self.base_url, timeout=self.timeout)
            if self.collect_mooncake or self.collect_gds:
                if self.before["server_config"].get("hicache_storage_backend") != "mooncake":
                    raise ValueError("Mooncake I/O metrics require the Mooncake storage backend")
            if self.collect_mooncake:
            ...
            if self.collect_gds:
                ...
                self.gds_before = fetch_gds_io_window(self.base_url, "begin", self.gds_window_id, self.flush_timeout)
    ...
    async def __aexit__(self, exc_type, exc, tb):
        ...
        if exc_type is None:
            try:
                self.after = fetch_hicache_io_snapshot(self.base_url, timeout=self.timeout)
                ...
                self.host_result = {
                    "hicache_io_status": "unavailable", "hicache_io_error": str(error),
                    "dram_read_bw_gbps": None, "dram_write_bw_gbps": None,
                    "mean_l2_kv_readback_ms": None,
                    "hicache_io_read_bytes": None, "hicache_io_write_bytes": None,
                    "hicache_io_read_batches": None, "hicache_io_write_batches": None,
                    "hicache_io_metadata": {"before": self.before, "after": self.after, "error": str(error)},
                }
        if self.collect_gds and exc_type is None:
            try:
                self.gds_after = fetch_gds_io_window(self.base_url, "end", self.gds_window_id, self.flush_timeout)
                self.gds_result = calculate_gds_io_metrics(self.gds_before, self.gds_after, self.gds_window_id)
                self.gds_begin_attempted = False
            except (requests.RequestException, ValueError, TimeoutError) as error:
                self.gds_result = unavailable_gds_io_metrics(error)
                self.gds_result["gds_io_metadata"] = {"before": self.gds_before, "after": self.gds_after}
        self._abort_gds()
        if self.ssd_before is not None:
            try:
                self.ssd_after = fetch_mooncake_io_snapshot(
                    self.client_url, "end", self.ssd_before["window"]["id"], self.ssd_before["pid"]
                )
                if exc_type is None:
                ...
                self.mooncake_result["mooncake_io_metadata"] = {
                    "ssd_before": self.ssd_before, "ssd_after": self.ssd_after,
                    "tp_before": self.before, "tp_after": self.after,
                }
                if exc_type is not None:
                    warnings.warn(f"Could not close this benchmark's Mooncake I/O window: {error}")
        if self.collect_mooncake and exc_type is None:
            try:
                self.mooncake_result.update(fetch_mooncake_storage_capacity(
                    args.mooncake_master_host, args.mooncake_metrics_port
                ))
            except (requests.RequestException, ValueError) as error:
                self.mooncake_result.setdefault("mooncake_io_metadata", {})["capacity_error"] = str(error)
        return False
        ...
        if self.collect_host:
            ...
            if self.host_result["hicache_io_status"] == "ok":
                ...
                for direction in ("read", "write"):
                    ...
                    print(f"L2 Host DRAM {direction} bandwidth (GB/s): {value:.4f}" + (" (no I/O)" if not batches else ""))
                    print(f"L2 Host {direction} rank-batches: {batches}; total (GB): {byte_count / 1e9:.4f}")
                value = self.host_result["mean_l2_kv_readback_ms"]
                print(f"Mean all-layer H2D rank-batch time (ms): {value if value is not None else 'N/A (no readbacks)'}")
            else:
...
class FlatMemoryIOWindow:
    """FLAT_MEMORY: Own one UUID window; telemetry must not discard request results."""

    def __init__(self, base_url, enabled):
        self.base_url = base_url
        ...
        self.result = dict.fromkeys((
            ...
            "flat_ssd_write_bw_gbps", "flat_ssd_read_peak_bw_gbps",
            "flat_ssd_write_peak_bw_gbps", "flat_ssd_read_io_count",
            "flat_ssd_write_io_count", "flat_ssd_durable_write_batches",
    ...
    async def _request(self, action=None):
        timeout = float(os.environ.get("HICACHE_FLUSH_TIMEOUT", "150")) if action else 15
        async with aiohttp.ClientSession(
            timeout=aiohttp.ClientTimeout(total=timeout)
        ) as session:
            kwargs = {"headers": get_auth_headers()}
            if action:
                kwargs["json"] = {"action": action, "window_id": self.window_id}
            async with session.request(
                "POST" if action else "GET",
                self.base_url + ("/flat_memory/io_window" if action else "/server_info"),
                **kwargs,
            ) as response:
                response.raise_for_status()
    ...
    async def _wait_idle(self):
        ...
        while True:
            ...
            # Idle checks only, outside request timing; never sample 100ms bandwidth.
    ...
    async def __aenter__(self):
        if self.enabled:
            try:
                ...
                # Set ownership before POST: a lost begin response still needs abort.
...
async def benchmark(
    ...
    native_io = NativeIOWindow(base_url, backend)
    ...
    if ("sglang" in backend and _get_bool_env_var("SGLANG_IS_IN_CI")) or flush_cache:
        requests.post(
            ...
            headers=get_auth_headers(),
            timeout=float(os.environ.get("HICACHE_FLUSH_TIMEOUT", "150")),
        ).raise_for_status()
    ...
    # FLAT_MEMORY: Begin after warmup/flush/idle; end and drain outside this timer.
    async with native_io, FlatMemoryIOWindow(
        base_url, getattr(args, "collect_flat_memory_io", False)
    ) as flat_io:
        # Run all requests
    ...
    # FLAT_MEMORY: A failed telemetry query must not discard completed requests.
    server_info = None
    accept_length = None
    try:
        response = requests.get(
            base_url + "/get_server_info", headers=get_auth_headers(), timeout=15
        )
        response.raise_for_status()
        server_info = response.json()
        if not isinstance(server_info, dict):
            raise ValueError("Server info is not a JSON object")
        if "sglang" in backend:
            server_info_json = server_info
            if "decode" in server_info_json:
                server_info_json = server_info_json["decode"][0]
            if server_info_json.get("internal_states"):
                accept_length = server_info_json["internal_states"][0].get(
                    "avg_spec_accept_length", None
                )
    except Exception as exc:
        server_info = None
        warnings.warn(f"Server info unavailable; preserving request results: {exc}")

    if native_io.collect_host:
    ...
    # FLAT_MEMORY: Legacy device/host/storage totals retain their original meaning.
    flat_mode = (
        flat_io.enabled
        or any(output.flat_dram is not None for output in outputs)
        or bool(((server_info or {}).get("internal_states") or [{}])[0].get("flat_memory"))
    )
    flat_cache = summarize_flat_cache(outputs, metrics.total_prompt_tokens)
    tiered_mode = (
        getattr(args, "collect_mooncake_gds_cache_metrics", False)
        or any(output.cache_source_mode == "mooncake_tiered_gds" for output in outputs)
    )
    tiered_cache = summarize_tiered_cache(outputs) if tiered_mode else None

    def print_flat_metric(label, value):
    ...
    if tiered_mode:
    ...
    elif metrics.total_cached_tokens > 0 or metrics.total_prompt_tokens > 0:
        ...
        if flat_mode:
            # FLAT_MEMORY: Host here means Flat DRAM, not the legacy HiCache pool.
            for label, tokens, rate in (
                ("Device(GPU HBM)", _flat_optional_sum(
                    [o for o in outputs if o.success], "server_cached_tokens_device"
                ), "device_hit_rate_pct"),
                ("Host(Flat DRAM)", flat_cache["flat_cached_tokens_dram"], "flat_dram_hit_rate_pct"),
                ("Storage(Flat NVMe SSD)", flat_cache["flat_cached_tokens_ssd"], "flat_ssd_hit_rate_pct"),
                ("FlatCake storage(DRAM+SSD) subtotal", flat_cache["flat_cached_tokens_total"], "flat_storage_hit_rate_pct"),
            ):
                print_flat_metric(f"{label} tokens:", tokens)
                print_flat_metric(f"{label} hit rate (%):", flat_cache[rate])
            print_flat_metric("Mixed tokens (subset of SSD, not additive):", flat_cache["flat_cached_tokens_mixed"])
        elif metrics.total_cached_tokens_device > 0 or metrics.total_cached_tokens_host > 0 or metrics.total_cached_tokens_storage > 0:
    ...
    if flat_mode:
        print("{s:{c}^{n}}".format(s="Flat Prefetch Arrival-to-All-TP-GPU-Ready", n=50, c="-"))
        for label, key in (
            ("All successful prefetch ops (ms):", "flat_prefetch_latency_ms"),
            ("DRAM-only prefetch ops (ms):", "flat_dram_prefetch_latency_ms"),
            ("SSD-dependent prefetch ops (ms):", "flat_ssd_prefetch_latency_ms"),
        ):
            print_flat_metric(label, flat_cache[key])
        print("{s:{c}^{n}}".format(s="Flat Completed/Durable I/O Window", n=50, c="-"))
        print_flat_metric("Flat I/O status:", flat_io.result["flat_io_status"])
        if flat_io.result["flat_io_error"]:
            print_flat_metric("Flat I/O error:", flat_io.result["flat_io_error"])
        for label, key in (
            ("I/O window (s):", "flat_io_window_seconds"),
            ("DRAM read bandwidth (GB/s):", "flat_dram_read_bw_gbps"),
            ("DRAM write bandwidth (GB/s):", "flat_dram_write_bw_gbps"),
            ("SSD read bandwidth (GB/s):", "flat_ssd_read_bw_gbps"),
            ("SSD durable write bandwidth (GB/s):", "flat_ssd_write_bw_gbps"),
            ("SSD read peak (100ms, GB/s):", "flat_ssd_read_peak_bw_gbps"),
            ("SSD durable write peak (100ms, GB/s):", "flat_ssd_write_peak_bw_gbps"),
            ("SSD completed read I/O count:", "flat_ssd_read_io_count"),
            ("SSD completed write I/O count:", "flat_ssd_write_io_count"),
            ("SSD durable write batches:", "flat_ssd_durable_write_batches"),
            ("SSD new KV blocks (rank/layer/K/V keys):", "flat_ssd_new_kv_blocks"),
        ):
            print_flat_metric(label, flat_io.result[key])
    ...
    # FLAT_MEMORY: These legacy transfer labels do not describe the Flat path.
    if not flat_mode and (metrics.avg_storage_read_latency_ms > 0 or metrics.total_storage_read_tokens > 0
    ...
    mooncake_eviction = {}
    if not flat_mode and not native_io.collect_mooncake and getattr(args, "mooncake_master_host", None):
    ...
    bandwidth_metrics = {}
    if not flat_mode and not native_io.collect_host and getattr(args, "prefill_metrics_host", None):
    ...
    if fm_host and not flat_mode and not native_io.collect_host:
    ...
    if (
        ...
        # FLAT_MEMORY: Separate Flat report from legacy totals and service BW metadata.
        if flat_mode:
    ...
    # FLAT_MEMORY: Keep per-request sums/op counts for auditable weighted latency.
    if flat_mode:
    ...
    if tiered_mode:
        result_details["tiered_cached_tokens_details"] = [output.tiered_cache for output in outputs]
        result_details["cache_source_modes"] = [output.cache_source_mode for output in outputs]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--collect-hicache-io-metrics", action="store_true",
        help="Collect completed all-TP native HiCache Host copy bytes and CUDA event timing.",
    )
    parser.add_argument(
        "--collect-mooncake-gds-io", action="store_true",
        help="Collect all-TP consumer cuFile read bandwidth and aligned 100 ms peaks after warmup/flush; requires the GDS-window runtime.",
    )
    parser.add_argument(
    ...
    # FLAT_MEMORY: Native 100ms buckets, not HTTP bandwidth sampling.
    parser.add_argument(
        "--collect-flat-memory-io",
        action="store_true",
        help="Collect a TP4 Flat I/O UUID window after warmup/flush through pipeline drain; "
        "telemetry/drain time is excluded from request throughput timing.",
    )
    parser.add_argument(
        "--flush-cache",
        action="store_true",
