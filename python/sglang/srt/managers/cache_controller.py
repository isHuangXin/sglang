...
class HiCacheController:
    ...
    def __init__(
        ...
        self._host_io_metrics = HostIOMetrics(enabled=self.enable_host_io_metrics)
    ...
    @property
    def _host_io_generation(self):
        return self._host_io_metrics.generation
    ...
    def host_io_snapshot(self) -> dict:
        ...
        if self.tiered_gds_mode:
            per_token, host_bytes, device_bytes = self._host_io_capacities()
            capacities = dict(bytes_per_token=per_token,
                              host_capacity_bytes=host_bytes,
                              device_capacity_bytes=device_bytes)
        else:
            capacities = host_pool_capacities(
    ...
    def _start_storage_threads(self):
        ...
        if self.tiered_gds_mode:
            ...
            self._storage_threads = [self.backup_thread]
            if not self.tiered_runtime_managed:
                self.prefetch_thread = threading.Thread(
                    target=self._tiered_query_worker, name="gds-query", daemon=True
                )
                self.prefetch_io_aux_thread = threading.Thread(
                    target=self._tiered_io_worker, name="gds-read", daemon=True
                )
                self._storage_threads.extend(
                    [self.prefetch_thread, self.prefetch_io_aux_thread]
                )
            for worker in self._storage_threads:
                worker.start()
            return
