"""Read-only, rank-local HiCache completion accounting and collection."""
...
class HiCacheIOCounters:
    ...
    def account_completed(self, direction: str, ack: HiCacheAck) -> float | None:
        # The caller consumed the ACK after its normal completion check;
        # accounting must never query, wait for, or synchronize an event.
        duration_ms = None
...
def snapshot_unified_hicache(cache) -> dict:
    ...
    if cache.tiered_runtime is not None:
        result["pending"]["storage_prefetch"] += len(cache.tiered_runtime.pending)
    ...
    if cc.enable_storage:
        ...
        if isinstance(cc.storage_backend, MooncakeStore):
            ...
            if cc.storage_backend.tiered_gds_mode:
                native = cc.storage_backend.get_gds_io_snapshot()
                result["mooncake_gds"] = {
                    "schema_version": native.get("schema_version"),
                    "mode": "compat",
                    "enabled": native.get("enabled") is True,
                    "ssd_io_available": native.get("ssd_io_available") is True,
                    "pid": native.get("pid"),
                    "gpu_id": cc.storage_backend.gds_device,
                    "ssd_root": cc.storage_backend.gds_config["gds_ssd_root"],
                    "window": native.get("window"),
                }
    return result
...
def collect_hicache_io(
    *,
    ...
    # Management fanout is guaranteed only for this topology;
    # other control routing must not introduce a cross-DP/PP collective.
